...
class OrganizationDataReporter:
    ...
    def __init__(self):
        ...
        self.colors = {
            'header_bg': '1E2761',      # Dark navy — Executive Summary only
            'header_bg_data': '366092', # Medium blue — matches Weekly report data sheet headers
    ...
    def _create_executive_summary(self, writer, org_data: Dict[str, Any]):
        ...
        def _get_setting(attr: str, default: str = '') -> str:
            """Read a setting — all metadata attrs are now proper @property methods
            that read directly from os.getenv(), so getattr() always returns the
            live .env value, never a Pydantic FieldInfo object."""
        ...
        developed   = _get_setting('developed_by',    'Security Engineering Team')
        version     = _get_setting('tool_version',    '1.0.0')
        copy_year   = _get_setting('copyright_year',  '') or str(datetime.now().year)
        website     = _get_setting('company_website',  '')
        github_repo = _get_setting('tool_github_repo', '')
        support_email = _get_setting('support_email',  '')
        ...
        if website:
            about_rows.append(('Company Website', website))
        if github_repo:
            about_rows.append(('Tool Repository', github_repo))
        if support_email:
            about_rows.append(('Support Email', support_email))
        about_rows += [
        ...
        for label, value in about_rows:
            ...
            # Hyperlink styling for URLs and email addresses
    ...
    def _create_security_summary_sheet(self, writer, org_data: Dict[str, Any]):
        ...
        for org in org_data.get('organizations', []):
            ...
            for repo in org.get('repositories', []):
                ...
                all_repos_health.append({
                    ...
                    'Admins / Owner': repo.get('repository_admins', 'N/A'),
                    'Admin Email': repo.get('admin_emails', 'N/A'),
                    'Severity': severity,
        ...
        if all_repos_health:
            ...
            df = pd.DataFrame(columns=['Organization', 'Repository', 'Admins / Owner', 'Admin Email', 'Severity', 'Compliance %'])
            df.to_excel(writer, sheet_name='Repository Health', index=False)
    ...
    def _apply_formatting(self, filename: Path, org_data: Dict[str, Any]):
        ...
        # Two header styles:
        #   data_header  — used on All Repositories, Repository Health, Organization Risk Pivot
        #                  matches the Weekly report's data sheet header (366092 medium blue)
        #   exec_header  — kept for any other sheets that may be added later (1E2761 dark navy)
        data_header_fill = PatternFill(start_color=self.colors['header_bg_data'],
                                       end_color=self.colors['header_bg_data'],
                                       fill_type='solid')
        exec_header_fill = PatternFill(start_color=self.colors['header_bg'],
                                       end_color=self.colors['header_bg'],
                                       fill_type='solid')
        header_font = Font(bold=True, color=self.colors['header_text'], size=11)

        # Sheets that get the Weekly-report header color
        data_color_sheets = {'All Repositories', 'Repository Health', 'Organization Risk Pivot'}
        ...
        for sheet_name in wb.sheetnames:
            ...
            # Choose header fill: weekly-report blue for data sheets, navy for others
            hdr_fill = data_header_fill if sheet_name in data_color_sheets else exec_header_fill
            ...
            if sheet_name == 'Repository Health':
                ...
                # Fixed moderate widths for the first four columns so the sheet
                # is readable immediately on open without horizontal scrolling.
                # These override the auto-adjust loop below for this sheet.
                ws.column_dimensions['A'].width = 22  # Organization
                ws.column_dimensions['B'].width = 28  # Repository
                ws.column_dimensions['C'].width = 22  # Admins / Owner
                ws.column_dimensions['D'].width = 28  # Admin Email
            elif sheet_name == 'All Repositories':
                self._format_repositories_sheet(ws)
                # Fixed moderate widths — auto-adjust was producing extreme values
                # (e.g. Full Name = 80, URL = 80) that forced horizontal scrolling.
                # These widths are sized to show content without excessive whitespace.
                fixed_widths = {
                    'A': 22,   # Organization
                    'B': 28,   # Repository Name
                    'C': 38,   # Full Name
                    'D': 32,   # Description
                    'E': 22,   # Owner
                    'F': 18,   # Primary Language
                    'G': 12,   # Visibility
                    'H': 10,   # Status
                    'I':  8,   # Fork
                    'J': 12,   # Size (KB)
                    ...
                    'L':  8,   # Forks
                    'M': 16,   # Default Branch
                    'N': 17,   # Days Since Push
                    'O': 13,   # Created
                    'P': 15,   # Last Updated
                    'Q': 13,   # Last Push
                    'R': 20,   # License
                    'S': 42,   # URL
                    'T': 20,   # Dependabot Alerts
                    'U': 22,   # Code Scanning Alerts
                    'V': 24,   # Secret Scanning Alerts
                    'W': 23,   # Total Security Alerts
                }
            ...
            # Auto-adjust column widths (for sheets without fixed widths)
            for column in ws.columns:
                max_length = 0
                column_letter = get_column_letter(column[0].column)

                # Skip columns with fixed widths
                if sheet_name == 'Repository Health' and column_letter in ['A', 'B', 'C', 'D']:
                    continue  # Already set above
                if sheet_name == 'All Repositories':
                    continue  # All columns have fixed widths above
